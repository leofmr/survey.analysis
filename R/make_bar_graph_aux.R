    ...
    ggplot2::geom_text(data = tidy_data,
                       aes(x = sub_question,
                           ...
                           label = ifelse(abs(percent) > label_thereshold,
                                          ...
                                          "")),
                       position = position_stack(vjust = alignment),
                       size = text_size)
  } else {
    NULL
  }
...
#' @import ggplot2
#'
#' @param grey_scale Logical. Indiativo se a escala utilizada é em escala de cinza ou não
...
#' @author Leonardo Rocha
#'
gen_fill_scale <- function(grey_scale,
                           palette_color,
                           direction_color = 1,
                           grey_start = 0.7,
                           grey_end = 0.3) {
